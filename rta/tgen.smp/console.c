...
int print_register_row(int index, smp *xcore)
{
   int           xx = 8;

   if (flag['e'-'a']) printf("[%p]", xcore->register_set);
   printf("%2.2x:", index);
...
int print7_registers(int index, smp *xcore)
{
   int		 xx = 7;
...
void action(char request[], smp *xcore)
{
   word			 sample;
   word			*departure;
   ...
   if (symbol == '=')
   {
      ...
      else if (symbol == '=') which = 0;
      else if ((symbol & 15) < RTA_SMP)
      {
         which = symbol;
         xcore = core + symbol;
         request+= 2;
   ...
   if (symbol == 'i')
   {
      ...
      quantum = total_delta;
      average /= quantum;
      printf("core %d instructions %lld usecs %lld approximate MIPS %f\n",
              base[77],
   ...
   switch(symbol)
   {
	 ...
	 general_indication |= LOCKSTEP;
      ...
      case 'g':
         ...
         if (xx == 2)
         {
            if (index | offset) breakpoint = &memory[index].w[offset];
            else breakpoint = NULL;
         }

         flag['s'-'a'] = 0;
         general_indication &= -1 ^ LOCKSTEP;

         if (breakpoint)
         {
            general_indication |= BREAKPOINT;
            ...
            if (flag['v'-'a']) printf(FORMAT1,
                                      general_indication, breakpoint - memory[0].w);
            if (flag['f'-'a']) printf("[@%p:%p]", memory[0].w, breakpoint);
            #endif
         }
         else general_indication &= -1 ^ BREAKPOINT;
      ...
      case 'm':
         ...
         if (xx < 2)
         {
            ...
            for (;;)
            {
               ...
               if (device_index == 0)
               {
                  pagep = memory + (base_index & 0x003FFFFF);
               ...
               while (offset < index)
               {
                  if (device_index)
                  {
                     if (base_index & 0x00800000) printf(" ******");
                                 ...
                                 device_read(device_index, base_index, pointer, 0, xcore));
                  }
                  else
                  {
                     sample = *wordp++;
         ...
         else
         {
            ...
            pagep = memory + base_index;
            wordp = pagep->w + offset;
            limitp = memory[PAGES_IN_MEMORY].w;
	...
	*****************************************************/
         ...
         for (;;)
         {
            ...
            while (offset < index)
            {
               ...
               xx = device_read(device_index, block, pointer, 0, xcore);
               printf(" %6.6x", xx);
               offset++;
               pointer++;
            }
	 ...
	 printf("%8.8x:%8.8x:%8.8x:%8.8x:%8.8x:%8.8x:%8.8x:%8.8x\n", 
		ultra,  ultra1, ultra2, ultra3,
		ultra4, ultra5, ultra6, ultra7);
	 break;

      case 'p':
	 base_index = offset = limitb = device_index = 0;
	 xx = sscanf(&request[1], "%x:%x/%d,%d",
                     &base_index, &offset, &limitb, &device_index);

	 if (xx == 0)
         {
            general_indication &= -1 ^ TOUCHPOINT;
	    touchpoint = touchpoint2 = NULL;
            printf("touchpoint cleared\n");
            break;
         }
         ...
         if ((device_sense >> 22) ^ 2)
         {
            printf("device %d not a system memory type\n", device_index);
            break;
         }

         if ((device_sense & 0x003FFFFF)
         <   (base_index + ((offset + limitb) >> 12)))
         {
            printf("page + offset/words %x:%x/%d beyond device %d\n",
                    base_index, offset, limitb, device_index);
            break;
         }

         touchpoint = devices[device_index].pages[base_index].w + offset;
         touchpoint2 = touchpoint + limitb;	 
         general_indication |= TOUCHPOINT;
