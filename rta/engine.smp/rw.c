...
int				 rta_mbanks=8;
int				 pages_in_memory = 8 * 64;
long				 words_in_memory = 8 * 64 * 4096;
...
static int   mread(word *w24p, int designator, smp *xcore);
static void mwrite(word *w24p, int designator, int write_value, smp *xcore);
...
int device_read(int device_index, int relocation_base, unsigned offset, int designator, smp *xcore)
{
   msw			*w24p;
   word16		*w16p;
   ...
   int			 v;
   int			 attribute = base[device_index + 128];
//   int			 device_type = base[128 + device_index] & 0x00C00000;
   int                   device_type = attribute & 0x00C00000;
   ...
   #if 0
   printf("\n[%x %p %x %x %x]", base[77], xcore, device_index, attribute, device_type);
   #endif
   ...
   else if (device_type ==SYSMEM_FLAG)
   {
      ...
      **************************************************/
      ...
      #endif
      {
         ...
         v = mread(pagep->w + offset, designator, xcore);
      }
   }
   else v = -4;
...
word memory_read(unsigned ea, smp *xcore)
{
...
//   static word	 nop			  = { 0, SK, 0, 128 + 1       } ;
   ...
   pagep = memory + relocation_base;
   wordp = pagep->w + offset;

//   printf("[%p %p %p %p]\n", &memory, &memory[PAGES_IN_MEMORY], pagep, wordp);
...
word *memory_hold(unsigned ea, smp *xcore)
{
   unsigned      relocation_base;
...
#define	OPERAND_READ1(EA) operand_read(EA, 7, xcore)

int operand_read(unsigned ea, int designator, smp *xcore)
{
   ...
   ********************************************************/
   ...
   if (relocation_base & 0x00400000)
   {
      ...
      **************************************************/
      ...
      if ((device_index = relocation_base & 63))
      {
         ...
         return device_read(device_index, relocation_base, offset, designator, xcore);
      }
   ...
   relocation_base &=0x003FFFFF;
   pagep = memory + relocation_base;
   return mread(pagep->w + offset, designator, xcore);
}

int burst_read2(int *list, unsigned ea, smp *xcore)
{
   ...
   if (relocation_base & 0x00400000)
   {
      ...
      **************************************************/
      ...
      if ((device_index = relocation_base & 63))
      {
         ...
         if ((temp = device_read(device_index, relocation_base, offset, 7, xcore)) < 0) return temp;
         *list = temp;
         if ((temp = device_read(device_index, relocation_base, offset + 1, 7, xcore)) < 0) return temp;
         *(list + 1) = temp;
         return 0;
      }
...
#ifdef	BANK_EDGE_GUARD
static int straddle_read4(unsigned offset, int *list, unsigned ea, smp *xcore)
{
   int		 sense;

   if ((sense = burst_read2(list, ea, xcore)) < 0) return sense;
   if ((sense = burst_read2(list + 2, ea + 2, xcore)) < 0) return sense;
   return 0;
}
#endif

int burst_read4(int *list, unsigned ea, smp *xcore)
{
      ...
      **************************************************/
      ...
      if (offset > 0x0003FFFC)
      {
         if (base[index + 1] ^ (relocation_base + 64))
         {
            ...
            straddle_read4(offset, list, ea, xcore);
            return 0;
         }
      }
      #endif
      ...
      if ((device_index = relocation_base & 63))
      {
         ...
         *list = temp;
         if ((temp = device_read(device_index, relocation_base, offset + 1, 7, xcore)) < 0) return temp;
         *(list + 1) = temp;
         if ((temp = device_read(device_index, relocation_base, offset + 2, 7, xcore)) < 0) return temp;
         *(list + 2) = temp;
         if ((temp = device_read(device_index, relocation_base, offset + 3, 7, xcore)) < 0) return temp;
         *(list + 3) = temp;
         return 0;
      }
      ...
      else
      {
         if (offset > 0x00000FFC)
         {
            if (base[index + 1] ^ (relocation_base + 1))
            {
               ...
               return straddle_read4(offset, list, ea, xcore);
            }
         }
      }
      #endif
...
static void device_write(int v, int device_index, int relocation_base, unsigned offset, int designator, smp *xcore)
{
   msw			*w24p;
   word16		*w16p;
   ...
   if (device_type == DATA16_FLAG)
   {
    ...
    else if (device_type == SYSMEM_FLAG)
    {
        ...
        **************************************************/
        ...
        {
           pagep = devicep->pages + relocation_base;
           mwrite(pagep->w + offset, designator, v, xcore);
        }
    }
}

#define OPERAND_WRITE1(DATA, EA) operand_write(DATA, EA, 7, xcore)

int operand_write(int v, unsigned ea, int designator, smp *xcore)
{
   unsigned		 index, relocation_base, device_index;
   unsigned		 alternate = 0;
   unsigned		 offset;
   ...
   if (relocation_base & 0x00400000)
   {
      ...
      **************************************************/
      ...
      if ((device_index = relocation_base & 63))
      {
         ...
         device_write(v, device_index, relocation_base, offset, designator, xcore);
         return 0;
      }
...
extern word	*touchpoint,
		*touchpoint2;

static void mwrite(word *w24p, int designator, int v, smp *xcore)
{
   #ifdef APPLY_TOUCHPOINT
   if (general_indication & TOUCHPOINT)
   {
      if (w24p < touchpoint)
      {
      }
...
int burst_write2(int *list, unsigned ea, smp *xcore)
{
   unsigned		 device_index, index, relocation_base;
   unsigned		 alternate = 0;
   unsigned		 offset;
   ...
   if (relocation_base & 0x00400000)
   {
      ...
      **************************************************/
      ...
      if ((device_index = relocation_base & 63))
      {
         ...
         device_write(v, device_index, relocation_base, offset, 7, xcore);
         device_write(w, device_index, relocation_base, offset + 1, 7, xcore);
         return 0;
...
#ifdef BANK_EDGE_GUARD
static int straddle_write4(unsigned offset, int *list, unsigned ea, smp *xcore)
{
   int		 sense;

   if ((sense = burst_write2(list, ea, xcore)) < 0) return sense;
   if ((sense = burst_write2(list + 2, ea + 2, xcore)) < 0) return sense;
   return 0;
}
#endif

int burst_write4(int *list, unsigned ea, smp *xcore)
{
   unsigned		 device_index, index, relocation_base;
   unsigned		 alternate = 0;
   unsigned 		 offset;
   ...
   if (relocation_base & 0x00400000)
   {
      ...
      **************************************************/
      ...
      if (offset > 0x0003FFFC)
      {
         if(base[index + 1] ^ (relocation_base + 64))
         {
            ...
            ************************************************/
            ...
            return straddle_write4(offset, list, ea, xcore);
         }
      }
      #endif
      ...
      if ((device_index = relocation_base & 63))
      {
         ...
         device_write(v,  device_index, relocation_base, offset, 7, xcore);
         device_write(w,  device_index, relocation_base, offset + 1, 7, xcore);
         device_write(_u, device_index, relocation_base, offset + 2, 7, xcore);
         device_write(_l, device_index, relocation_base, offset + 3, 7, xcore);
         return 0;
      }
...
void store24(int *where, int what)
{
   ORDER32(*where, what)
}
